...
export default new Vuex.Store({ // 
    ...
    getters: { //  相当于我们的计算属性 和计算属性差不多 它更改的也是数据池里面的东西 通过数据池里面的某个值进行更改来得到新的一个值
    ...
    actions: { // 可以在里面触发某个Mutation的函数 让它去执行这个函数 然后通过里面的函数去更改里面的值 在组件内就能触发Mutation里的函数 为什么还要用action呢 它存在的意义就是为了执行异步的操作 如果是要异步的话就放在action里面
        // changeStudent(ctx, {name, number}) { // 上下文对象 是最外面的对象 相当于store
        //     setTimeout(function() { // 异步延迟1000毫秒执行
        //         ctx.commit('changeStudent', {name, number});
        //     }, 1000)   
