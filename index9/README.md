...
非父子组件传值
1. 父子传值 子传父 父传子 仅限一层级 并且两个之间去传值
2. 直接传值：事件组件 在vue的实例上面 不用经过父组件了就能拿到兄弟数据  数据少量传递的时候 这个数据组件就可以利用这个方式 组件多了也很麻烦 有没有某种方式让我去更改某个地方的值 我很多个组件都能去知道我这个数据发生改变也能进行响应 也能把更改的数据显示在我们的页面上 vuex 可以把他当成一个仓库 组件共享仓库的某个数据 我在这个组件里面可以对这个数据进行一个更改 更改完成之后我不要做任何的处理 我在其他组件也能立刻显示出来我更改后的值 使用它需要安装它 
