...
---
#### 功能：从添加组件传值到学生列表然后让学生列表这个组件显示出来

- 非父子组件传值
  - 父子传值：子传父，父传子，仅限一层级，并且两个之间去传值。
  - 直接传值：事件组件，在vue的实例上面，不用经过父组件了就能拿到兄弟数据，数据少量传递的时候，这个数据组件就可以利用这个方式，组件多了也很麻烦。
  - vuex：有没有某种方式让我去更改某个地方的值，我很多个组件都能去知道我这个数据发生改变也能进行响应，也能把更改的数据显示在我们的页面上，vuex可以把他当成一个仓库，组件共享仓库的某个数据 我在这个组件里面可以对这个数据进行一个更改，更改完成之后我不要做任何的处理，我在其他组件也能立刻显示出来我更改后的值，使用它需要安装它。 
    - vue install vuex --save 多了个store.js
    - vuex 安装及处理
...
我们用getter实现的这个值可以在任何一个组件都能实现这个复用了 如果涉及到正常数据池里面的值进行更改的话就可以在getter里面来做
...
Vue mutation action
mutation函数去进行更改
以后我们要是想修改数据池里面的数据 一定要通过mutation函数
跳转第九次触发函数的时候
用vue-tool工具进行调试
...
---
利用vuex实现课程修改
两个组件共用一个数据并且在某个组件里面进行更改的时候另一个组件的视图也发生了改变 那我们需要用vuex来做
changeCourse 它不是learn下面的组件 所以在Components下新建一个组件
因为为批量处理 所以写一个template包裹起来
点击修改的会改变公共数据池里面的courseList 更改Vuex里面的数据要在Mutation来做
完成数据的实时更新
...
---
vuex module
如果写100个功能都需要用到vuex 是不是意味着 是不是意味着state里面很臃肿 同样getters也依然会很臃肿 并且每个功能的数据都是放到一个数据里 是不是更不方便我们去调试 有没有什么方法把一个数据里面的功能抽离出来成为一个store 各个功能的数据不互相影响 也不放在一起 也不成为一坨
如果说你想去取到某个模块里面的内容 需要在state后面加上这个模块的名字
